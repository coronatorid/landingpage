...
const ThemeToggle = (props) => {
  const {theme, switchTheme} = useContext(ThemeContext.Context);
      ...
      <label htmlFor="toggle" className="flex items-center cursor-pointer">
        <div className="relative">
          <input
            ...
            id="toggle"
            className="hidden"
            onChange={() => switchTheme(theme != 'dark' ? 'dark': 'normal')}
            name="theme"
            checked={theme == 'dark'}
            />
          ...
          <div
            className={`
             transition duration-300 ease-in-out -top-1 absolute w-6 h-6 bg-white rounded-full shadow inset-y-0 left-0 flex items-center justify-center
             ${theme == 'dark' ? 'transform translate-x-full bg-yellow-600' : 'bg-yellow-300'} `}
          >
              {iconTheme()}
          </div>
        </div>
